...
`go-ruby-tzinfo/tzinfo` reimplements Ruby's `tzinfo` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-tzinfo/tzinfo` directly, with no Ruby runtime;
